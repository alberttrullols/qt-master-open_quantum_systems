...
# ----------------------
# Hamiltonian and relaxation
# ----------------------
ω = 1.0           # rotation frequency (same)
Γ1 = 0.2          # relaxation rate
H = ω * σz
c_ops = [sqrt(Γ1) * σm]   # <-- relaxation instead of dephasing
...
anim = @animate for i in 1:length(tout)
    ...
    scatter!([0.0], [0.0], [0.0], color=:black, label=false)
    quiver!([0.0], [0.0], [0.0], quiver=([x[i]], [y[i]], [z[i]]),
            arrowsize=0.15, linewidth=2, color=:red)
    title!("Bloch vector under pure relaxation\n t = $(round(tout[i], digits=2))")
    xlims!(-1,1); ylims!(-1,1); zlims!(-1,1)
end

gif(anim, "bloch_relaxation.gif", fps=20)
println("Saved animation as 'bloch_relaxation.gif'")
